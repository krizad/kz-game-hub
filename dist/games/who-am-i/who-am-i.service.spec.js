...
const who_am_i_service_1 = require("./who-am-i.service");
...
describe('WhoAmIService', () => {
    ...
    beforeEach(async () => {
        ...
        const module = await testing_1.Test.createTestingModule({
            providers: [who_am_i_service_1.WhoAmIService, { provide: private_state_service_1.PrivateStateService, useValue: privateState }],
        }).compile();
        service = module.get(who_am_i_service_1.WhoAmIService);
        jest.clearAllMocks();
    });
    ...
    describe('getCategories', () => {
        it('should fetch and map categories from DB', async () => {
            database_1.prisma.word.groupBy.mockResolvedValue([
                { category: 'Animals', _count: { id: 5 } },
                { category: 'Food', _count: { id: 3 } },
            ]);
            const result = await service.getCategories();
            expect(database_1.prisma.word.groupBy).toHaveBeenCalledWith({
                by: ['category'],
                _count: { id: true },
                where: {},
            });
    ...
    describe('startGameRandom', () => {
        ...
        it('should return null if not enough words in DB', async () => {
            ...
            database_1.prisma.word.findMany.mockResolvedValue([{ word: 'Pizza', emoji: '🍕' }]);
            expect(await service.startGameRandom(room, 'host1')).toBeNull();
        });
    ...
    describe('submitPlayerWord', () => {
        ...
        it('should transition from COLLECTING_WORDS to ASKING when all players submit', () => {
            ...
            room.players.forEach((p) => {
                expect(assignedWords[p.socketId]).not.toBe(privateState.get('test-room', p.socketId, 'waiSubmittedWord'));
            });
        });
    });
    describe('handleGameAction', () => {
        function makeAskingRoom(overrides = {}) {
            return {
                code: 'test-room',
    ...
    describe('two-phase HOST_INPUT start', () => {
        it('accepts word submission after startGameAwaitHostInput flipped the room to PLAYING', () => {
            ...
            expect(service.startGameHostInput(room, 'host1', { p1: 'Firefighter', p2: 'Astronaut' })).not.toBeNull();
...
//# sourceMappingURL=who-am-i.service.spec.js.map