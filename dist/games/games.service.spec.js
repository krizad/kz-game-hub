...
describe('GamesService', () => {
    ...
    const mockGameServices = {
        ...
        ticTacToe: {
            ...
            makeMove: jest.fn(),
            reset: jest.fn(),
            remapSocketId: tic_tac_toe_service_1.TicTacToeService.prototype.remapSocketId,
        },
        rps: {
            assignRoles: jest.fn(),
            makeChoice: jest.fn(),
            nextRound: jest.fn(),
            reset: jest.fn(),
            remapSocketId: rps_service_1.RPSService.prototype.remapSocketId,
        },
        gobbler: {
            joinSide: jest.fn(),
            placePiece: jest.fn(),
            ...
            remapSocketId: gobbler_service_1.GobblerService.prototype.remapSocketId,
        },
        soundsFishy: {
            assignRoles: jest.fn(),
            typeAnswer: jest.fn(),
        ...
        whoAmI: {
            ...
            resetGame: jest.fn(),
            remapSocketId: who_am_i_service_1.WhoAmIService.prototype.remapSocketId,
        },
        whoFirst: {
            startGame: jest.fn(),
            handleGameAction: jest.fn(),
            setActive: jest.fn(),
            resetGame: jest.fn(),
            remapSocketId: who_first_service_1.WhoFirstService.prototype.remapSocketId,
        },
        musicTrivia: {
            startGame: jest.fn(),
            handleGameAction: jest.fn(),
            ...
            resetGame: jest.fn(),
            remapSocketId: music_trivia_service_1.MusicTriviaService.prototype.remapSocketId,
    ...
    beforeEach(async () => {
        ...
        const module = await testing_1.Test.createTestingModule({
            providers: [
                ...
                { provide: who_know_service_1.WhoKnowService, useValue: mockGameServices.whoKnow },
                { provide: tic_tac_toe_service_1.TicTacToeService, useValue: mockGameServices.ticTacToe },
                { provide: rps_service_1.RPSService, useValue: mockGameServices.rps },
                { provide: gobbler_service_1.GobblerService, useValue: mockGameServices.gobbler },
                { provide: sounds_fishy_service_1.SoundsFishyService, useValue: mockGameServices.soundsFishy },
                { provide: detective_club_service_1.DetectiveClubService, useValue: mockGameServices.detectiveClub },
                { provide: who_am_i_service_1.WhoAmIService, useValue: mockGameServices.whoAmI },
        ...
        whoKnowService = module.get(who_know_service_1.WhoKnowService);
        ticTacToeService = module.get(tic_tac_toe_service_1.TicTacToeService);
        rpsService = module.get(rps_service_1.RPSService);
        gobblerService = module.get(gobbler_service_1.GobblerService);
        ...
        const cardGameInstance = new card_game_service_1.CardGameService(privateStateService);
        mockGameServices.cardGame.cancelRound = jest.fn(cardGameInstance.cancelRound.bind(cardGameInstance));
        service.rooms.clear();
    ...
    describe('createRoom', () => {
        it('should create a room with default WHO_KNOW game type', () => {
            ...
            expect(room.gameType).toBe(types_1.GameType.WHO_KNOW);
        ...
        it('should create a card-game room with the Slave default config when selected', () => {
            const room = service.createRoom('host1', types_1.GameType.CARD_GAME, { cardGamePreset: 'SLAVE' });
            expect(room.gameType).toBe(types_1.GameType.CARD_GAME);
        ...
        it('should create an RPS room with initial state', () => {
            ...
            expect(room.gameType).toBe(types_1.GameType.RPS);
        ...
        it('should create a WHO_AM_I room with default config', () => {
            ...
            expect(room.gameType).toBe(types_1.GameType.WHO_AM_I);
        ...
        it('should create a Sounds Fishy room with no extra state (handled at start)', () => {
            ...
            expect(room.gameType).toBe(types_1.GameType.SOUNDS_FISHY);
            expect(room.status).toBe(types_1.RoomStatus.LOBBY);
        });
    ...
    describe('leaveRoom', () => {
        ...
        it('should restore the seat when the player rejoins within the grace window', () => {
            ...
            service.leaveRoom('p1', false);
            const rejoined = service.joinRoom(room.code, { id: 'p1-new', name: 'Player1', socketId: 'p1-new' }, token);
    ...
    describe('viewer mode', () => {
        ...
        it('should keep viewer status when a viewer reconnects', () => {
            ...
            const reconnectToken = service.getReconnectToken(room.code, 'p1');
            const updated = service.joinRoom(room.code, { id: 'p2', name: 'Late Joiner', socketId: 'p2' }, reconnectToken);
        ...
        it('should clear viewer flags when the game returns to LOBBY', () => {
            const room = startPlayingRoom();
            service.joinRoom(room.code, { id: 'p1', name: 'Late Joiner', socketId: 'p1' });
            whoKnowService.resetGame.mockReturnValue({ ...room, status: types_1.RoomStatus.LOBBY });
            const updated = service.resetGame(room.code, 'host1');
            expect(updated.status).toBe(types_1.RoomStatus.LOBBY);
            expect(updated.players.find((p) => p.socketId === 'p1').isViewer).toBeUndefined();
    ...
    describe('getAvailableRooms', () => {
        it('should return only LOBBY rooms', () => {
            ...
            playingRoom.status = types_1.RoomStatus.PLAYING;
            service.rooms.set(playingRoom.code, playingRoom);
    ...
    describe('updateConfig', () => {
        ...
        it('should reject a card-game config outside the preset allow-list', () => {
            ...
            const invalid = service.updateConfig(room.code, 'host1', {}, {
                scoring: { ...pok_deng_preset_1.POK_DENG_PRESET.defaultConfig.scoring, baseStake: 0 },
            });
    ...
    describe('assignRoles', () => {
        it('should delegate to SoundsFishyService for SOUNDS_FISHY game', async () => {
            ...
            const expectedResult = { room: resultRoom, roles: { p1: types_1.Role.Host } };
            ...
            expect(service.getRoom(room.code).status).toBe(types_1.RoomStatus.QUESTIONING);
        });
        it('should delegate to RPSService for RPS game', async () => {
            const room = service.createRoom('host1', types_1.GameType.RPS);
            const resultRoom = { ...room, status: types_1.RoomStatus.PLAYING };
        ...
        it('should delegate to DetectiveClubService for DETECTIVE_CLUB game', async () => {
            const room = service.createRoom('host1', types_1.GameType.DETECTIVE_CLUB);
            const updatedRoom = { ...room, status: types_1.RoomStatus.PLAYING };
        ...
        it('should default to WhoKnowService for other game types', async () => {
            const room = service.createRoom('host1', types_1.GameType.WHO_KNOW);
            const resultRoom = { ...room, status: types_1.RoomStatus.WORD_SETTING };
            const expectedResult = { room: resultRoom, roles: { host1: types_1.Role.Host } };
    ...
    describe('setWord', () => {
        it('should delegate to WhoKnowService and persist', () => {
            const room = service.createRoom('host1');
            const updatedRoom = { ...room, status: types_1.RoomStatus.QUESTIONING };
    ...
    describe('stopTimer', () => {
        it('should delegate to WhoKnowService', () => {
            const room = service.createRoom('host1');
            const updatedRoom = { ...room, status: types_1.RoomStatus.VOTING };
    ...
    describe('submitVote', () => {
        it('should delegate to WhoKnowService', () => {
            const room = service.createRoom('host1');
            const updatedRoom = { ...room, status: types_1.RoomStatus.RESULT };
    ...
    describe('ticTacToe delegation', () => {
        it('should delegate joinSide and persist updated room', () => {
            ...
            expect(service.getRoom(room.code).status).toBe(types_1.RoomStatus.PLAYING);
            expect(result).not.toBeNull();
        });
    ...
    describe('authorization and cleanup', () => {
        it('should let the server trigger The Mind timeout', () => {
            const room = service.createRoom('host1', types_1.GameType.THE_MIND);
        ...
        it('drops the saboteur auto-pass deadline when the room is deleted', () => {
            const room = service.createRoom('host1', types_1.GameType.SABOTEUR);
