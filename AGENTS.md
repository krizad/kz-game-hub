...
- **In-memory game state**: Room state lives in `Map<string, RoomState>` inside `GamesService`. Do not persist game state to DB during play. PostgreSQL is only used for reference/persistent data: Sounds Fishy trivia questions, Who Am I words, the leaderboard's recorded game results, and shared `CardRulePreset` rule codes.
