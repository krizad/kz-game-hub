      ...
      {room.status === RoomStatus.LOBBY ? (
        <div className="flex flex-col items-center gap-6">
          ...
          <div className="flex gap-4">
            <div className={`p-6 border-2 rounded-2xl flex flex-col items-center gap-4 ${ttt.playerXId ? 'border-amber-300 bg-white/50' : 'border-indigo-500 bg-indigo-950/20'}`}>
              <div className="text-4xl font-black text-blue-400">X</div>
              {ttt.playerXId ? (
                <div className="text-slate-700 font-bold">{room.players.find(p => p.socketId === ttt.playerXId)?.name}</div>
              ) : (
                <button onClick={() => tttJoinSide("X")} className="bg-indigo-600 hover:bg-indigo-500 px-4 py-2 rounded-lg font-bold">Join as X</button>
              )}
            </div>
            
            <div className={`p-6 border-2 rounded-2xl flex flex-col items-center gap-4 ${ttt.playerOId ? 'border-amber-300 bg-white/50' : 'border-rose-500 bg-rose-950/20'}`}>
              <div className="text-4xl font-black text-rose-400">O</div>
              {ttt.playerOId ? (
                <div className="text-slate-700 font-bold">{room.players.find(p => p.socketId === ttt.playerOId)?.name}</div>
              ) : (
        ...
        <div className="flex flex-col items-center gap-8 w-full max-w-md bg-white border border-amber-200 rounded-3xl p-6 shadow-2xl">
          ...
          <div className="flex justify-between w-full items-center px-4">
            <div className={`flex flex-col items-center ${ttt.currentTurn === "X" ? "scale-110 drop-shadow-[0_0_15px_rgba(96,165,250,0.5)]" : "opacity-50"} transition-all`}>
              ...
              <span className="text-slate-700 font-medium text-sm">{room.players.find(p => p.socketId === ttt.playerXId)?.name}</span>
            ...
            <div className={`flex flex-col items-center ${ttt.currentTurn === "O" ? "scale-110 drop-shadow-[0_0_15px_rgba(244,63,94,0.5)]" : "opacity-50"} transition-all`}>
              ...
              <span className="text-slate-700 font-medium text-sm">{room.players.find(p => p.socketId === ttt.playerOId)?.name}</span>
          ...
          {room.status === RoomStatus.RESULT && (
            <div className="flex flex-col items-center gap-4 animate-in zoom-in slide-in-from-bottom-4">
              {ttt.winner === "DRAW" ? (
                <div className="text-2xl font-black text-slate-600 bg-amber-100 px-6 py-2 rounded-xl border border-amber-300">It's a Draw!</div>
              ) : (
