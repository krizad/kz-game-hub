  ...
  return (
    <div className="flex-1 flex flex-col space-y-6">
      <div className="bg-white border border-amber-200 rounded-xl p-6 text-center w-full shadow-lg">
        ...
        {isConspirator ? (
          <div className="flex items-center justify-center">
            <span className="text-xl font-bold bg-amber-50 text-slate-500 px-6 py-2 rounded-lg border border-amber-200 tracking-[0.5em] blur-[2px] select-none pointer-events-none">????????</span>
          </div>
        ) : (
      ...
      <div className="flex-1 bg-amber-50/50 border border-amber-200 rounded-xl p-4 sm:p-6 overflow-x-auto min-h-[300px]">
        <h3 className="text-slate-600 font-bold uppercase tracking-widest text-xs mb-4 text-center">Played Cards</h3>
        <div className="flex flex-wrap gap-4 justify-center items-center">
          {state.playOrder.map(pid => {
            ...
            return (
              <div key={pid} className={`flex flex-col items-center p-3 rounded-xl border ${isActive ? 'bg-indigo-950/30 border-indigo-500/50 shadow-[0_0_15px_rgba(99,102,241,0.2)]' : 'bg-white/50 border-amber-200'} transition-all min-w-[120px]`}>
                <span className={`text-sm font-bold mb-2 truncate max-w-[100px] ${isActive ? 'text-indigo-400' : 'text-slate-700'}`}>{pName}</span>
                <div className="flex gap-2 min-h-[140px]">
                  ...
                  {Array.from({ length: (state.currentPhase === DetectiveClubPhase.PLAYING_ROUND_1 ? 1 : 2) - player.playedCards.length }).map((_, i) => (
                    <div key={`empty-${i}`} className={`w-20 h-28 sm:w-24 sm:h-32 rounded-lg border-2 border-dashed flex items-center justify-center ${isActive ? 'border-indigo-500/40 bg-indigo-500/5' : 'border-amber-200/80 bg-white/20'}`}>
      ...
      <div className="bg-white border border-amber-200 rounded-xl p-4 sm:p-6 shadow-2xl relative overflow-hidden">
        ...
        <div className={`absolute top-0 left-0 w-full py-1 text-center text-xs font-bold uppercase tracking-widest ${isMyTurn ? 'bg-indigo-600 text-white' : 'bg-amber-100 text-slate-600'}`}>
          {isMyTurn ? "Your Turn - Play a Card" : `Waiting for ${activePlayerName}...`}
        </div>

        <h3 className="text-slate-600 font-bold uppercase tracking-widest text-xs mb-4 text-center mt-6">Your Hand</h3>
      ...
      {confirmPlayIndex !== null && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4" onClick={() => setConfirmPlayIndex(null)}>
          <div className="bg-white border-2 border-amber-300 rounded-xl max-w-sm w-full p-6 text-center shadow-2xl transform scale-100 transition-transform" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-slate-600 mb-6 font-medium">Are you sure you want to play this card?</p>
