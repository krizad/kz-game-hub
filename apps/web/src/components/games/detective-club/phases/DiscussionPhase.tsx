      ...
      <div className="flex-1 bg-amber-50/50 border border-amber-200 rounded-xl p-4 sm:p-6 overflow-x-auto min-h-[300px]">
        ...
        <div className="flex flex-wrap gap-4 justify-center items-center">
          {state.playOrder.map(pid => {
            ...
            return (
              <div key={pid} className={`flex flex-col items-center p-3 rounded-xl border ${isMe ? 'bg-indigo-950/30 border-indigo-500/50' : 'bg-white/50 border-amber-200'} transition-all min-w-[120px]`}>
                <span className={`text-sm font-bold mb-2 truncate max-w-[100px] ${isMe ? 'text-indigo-400' : 'text-slate-700'}`}>{pName} {isMe && "(You)"}</span>
