...
function GameLobby() {
  ...
  const LanguageSwitcher = () => (
    <div className="flex gap-1 bg-amber-100/50 p-1 rounded-lg border border-amber-300/50 backdrop-blur-sm z-50">
      <button
        ...
        className={`px-2 py-1 text-[10px] sm:text-xs font-bold rounded-md transition-all ${language === 'en' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-600 hover:text-slate-800 hover:bg-amber-200/50'}`}
      >
        EN
      </button>
      <button
        onClick={() => setLanguage('th')}
        className={`px-2 py-1 text-[10px] sm:text-xs font-bold rounded-md transition-all ${language === 'th' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-600 hover:text-slate-800 hover:bg-amber-200/50'}`}
      >
        TH
      </button>
    </div>
        ...
        <main className="flex min-h-screen flex-col items-center justify-center p-4 sm:p-6 bg-amber-50 text-slate-800 relative">
          ...
          <div className="w-full max-w-md p-6 sm:p-8 bg-white border border-indigo-500/30 rounded-3xl shadow-2xl relative overflow-hidden">
            ...
            <div className="flex justify-center mb-4 mt-2">
              <img src="/icon.png" alt="KZ Game Hub Logo" className="w-20 h-20 rounded-2xl shadow-lg shadow-indigo-500/20 border border-amber-300" />
            </div>
            <h1 className="text-3xl sm:text-4xl font-black text-center mb-2 tracking-tighter text-slate-800">{t('lobby.invited')}</h1>
            ...
            <div className="space-y-6">
              <div>
                <label className="block text-sm font-medium text-slate-600 mb-2">{t('lobby.displayName')}</label>
                <input
                  type="text"
                  value={myName}
      ...
      <main className="flex min-h-screen flex-col items-center justify-center p-4 sm:p-6 bg-amber-50 text-slate-800 relative">
        ...
        <div className="w-full max-w-md p-6 sm:p-8 bg-white border border-amber-200 rounded-3xl shadow-2xl">
          <div className="flex justify-center mb-6">
            <img src="/icon.png" alt="KZ Game Hub Logo" className="w-24 h-24 sm:w-28 sm:h-28 rounded-[2rem] shadow-2xl shadow-indigo-500/20 border border-amber-300" />
          </div>
          ...
          <div className="space-y-6">
            <div>
              <label className="block text-sm font-medium text-slate-600 mb-2">{t('lobby.displayName')}</label>
              <input type="text" value={myName} onChange={(e) => setName(e.target.value)} className="w-full bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all font-medium" placeholder={t('lobby.enterNameShort')} />
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              ...
              <button onClick={() => createRoom(GameType.DETECTIVE_CLUB)} disabled={!myName} className="w-full bg-amber-200/80 hover:bg-amber-200 disabled:opacity-50 disabled:cursor-not-allowed text-slate-800 font-bold py-3 rounded-xl transition-colors shadow-lg border border-amber-400/50 flex flex-col items-center justify-center gap-1 group">
                <span className="text-xl group-hover:scale-110 transition-transform">🔍</span>
              ...
              <div className="space-y-3 max-h-[300px] overflow-y-auto pr-1">
                {availableRooms.map((r, i) => (
                  <button
                    ...
                    className="w-full bg-white border border-amber-200/80 hover:border-indigo-500/50 rounded-2xl p-4 text-left transition-all flex items-center justify-between group shadow-lg hover:shadow-indigo-500/10 hover:-translate-y-0.5"
                  >
                    <div>
                      ...
                      <div className="text-slate-500 text-[10px] font-medium uppercase mt-0.5 tracking-wider flex items-center gap-1.5">
                        ...
                        {t('lobby.host')} <span className="text-slate-700 normal-case font-bold">{r.hostName}</span>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      <div className="flex items-center gap-1.5 text-xs font-bold text-slate-600 bg-amber-50 px-2.5 py-1.5 rounded-lg border border-amber-200 shadow-inner group-hover:border-indigo-500/30 transition-colors" title={t('lobby.playersInRoom')}>
                        {r.playerCount}
  ...
  return (
    <main className="flex min-h-[100dvh] flex-col items-center p-2 sm:p-4 bg-amber-50 text-slate-800">
        ...
        <header className="flex-none flex items-center justify-between gap-4 p-2 sm:p-4 bg-white border border-amber-200 rounded-2xl shadow-xl z-10 w-full">
          <div className="flex items-center gap-2 sm:gap-3 flex-wrap sm:flex-nowrap w-full">
            ...
            <span className="text-[10px] sm:text-xs font-medium text-slate-500 ml-1 sm:ml-2 border-l border-amber-300 pl-2 sm:pl-4 py-0.5 flex items-center gap-1">
              ...
              <span className="text-slate-700 font-bold truncate max-w-[100px] sm:max-w-[150px]" title="Room Creator">
        ...
        {room.gameType === GameType.GOBBLER_TIC_TAC_TOE ? (
          ...
          <div className="flex-1 min-h-0 flex flex-col md:flex-row gap-2 sm:gap-4">
            ...
            <div className="flex flex-none items-center justify-between mb-2 sm:mb-3">
              <h3 className="text-xs font-bold text-slate-600 uppercase tracking-wider pl-1">{t('lobby.players')}</h3>
              <span className="bg-amber-100 px-2 py-0.5 rounded-full text-[10px] text-indigo-400 font-black border border-amber-300">{room.players.length}</span>
            </div>
                  ...
                  {room.players.map((p) => (
                    <tr key={p.id} className="bg-amber-100/10 hover:bg-amber-100/40 transition-colors">
                      <td className="px-3 py-2 font-medium flex items-center gap-2.5">
                        <span className="w-6 h-6 rounded-full bg-amber-100 flex items-center justify-center text-sm shadow-inner flex-shrink-0 border border-amber-300" title={p.name}>
                          {getAvatarEmoji(p.id)}
                        </span>
                        <span className="truncate max-w-[120px] sm:max-w-[200px] text-slate-700">
                          {p.name}
                          {p.connected === false && <span className="text-[9px] font-bold text-slate-500 ml-1.5 align-middle border border-amber-300 bg-amber-100/50 px-1 py-0.5 rounded leading-none inline-flex">({t('lobby.offline')})</span>}
          ...
          <div className="flex-none md:flex-[1.5] flex flex-col bg-white/80 border border-amber-200 rounded-2xl p-2 sm:p-4 shadow-xl min-h-[300px]">
              ...
              <div className="flex-1 flex flex-col items-center justify-center gap-6 min-h-[150px]">
                ...
                <div className="w-full max-w-sm bg-amber-50/50 border border-amber-200 rounded-xl p-4 space-y-4">
                  {room.gameType === GameType.WHO_KNOW && (
                    <>
                      <div>
                        <label className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-2">{t('lobby.hostSelection')}</label>
                        {useGameStore.getState().socketId === room.roomHostId ? (
                          <select value={room.config?.hostSelection || "ROUND_ROBIN"} onChange={(e) => useGameStore.getState().updateConfig({ hostSelection: e.target.value as any })} className="w-full bg-white border border-amber-300 rounded-lg px-3 py-2 text-sm text-slate-800 focus:outline-none focus:border-indigo-500 appearance-none">
                          ...
                          <div className="text-slate-700 font-medium text-sm px-3 py-2 bg-white/50 rounded-lg border border-amber-200/50">{room.config?.hostSelection === "ROUND_ROBIN" ? t('lobby.roundRobin') : room.config?.hostSelection === "RANDOM" ? t('lobby.random') : t('lobby.roomCreatorFixed')}</div>
                        )}
                      </div>
                      <div>
                        <label className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-2">{t('lobby.timerMinutes')}</label>
                        {useGameStore.getState().socketId === room.roomHostId ? (
                          <input
                            type="number"
                            min="1"
                          ...
                          <div className="text-slate-700 font-medium text-sm px-3 py-2 bg-white/50 rounded-lg border border-amber-200/50">{room.config?.timerMin || 5} {t('lobby.minutes')}</div>
                        )}
                      </div>
                    </>
                  )}
                  ...
                  {room.gameType === GameType.RPS && (
                    <>
                      <div>
                        <label className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-2">{t('lobby.mode')}</label>
                        {useGameStore.getState().socketId === room.roomHostId ? (
                          <select
                      ...
                      <div>
                        <label className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-2">{t('lobby.targetScore')}</label>
                        {useGameStore.getState().socketId === room.roomHostId ? (
                          <select
          ...
          <div className="bg-white border border-indigo-500/50 rounded-3xl w-full max-w-md shadow-2xl overflow-hidden relative animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-6 md:p-8 flex flex-col gap-6">
              <div className="text-center">
                <h3 className="text-2xl font-black text-slate-800 uppercase tracking-widest mb-2">{t('lobby.youAreHost')}</h3>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-amber-50/80 backdrop-blur-sm p-4">
          <div className="bg-white border border-rose-500/50 rounded-3xl w-full max-w-sm shadow-2xl overflow-hidden relative animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-6 md:p-8 flex flex-col gap-4 text-center">
              ...
              <h3 className="text-2xl font-black text-slate-800 uppercase tracking-widest">{t('lobby.leaveRoomTitle')}</h3>
              <p className="text-slate-600 font-medium text-sm mb-4 whitespace-pre-line">
                {t('lobby.leaveRoomDesc')}
              </p>

              <div className="flex gap-3 mt-2">
                <button onClick={() => setShowLeaveModal(false)} className="flex-1 bg-amber-100 hover:bg-amber-200 text-slate-700 font-bold py-3 px-4 rounded-xl transition-all">
                  {t('lobby.cancel')}
                </button>
                <button
                  onClick={() => {
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-amber-50/80 backdrop-blur-sm p-4">
          <div className="bg-white border border-purple-500/50 rounded-3xl w-full max-w-sm shadow-2xl overflow-hidden relative animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-6 md:p-8 flex flex-col items-center gap-4 text-center">
              <h3 className="text-2xl font-black text-slate-800 uppercase tracking-widest">{t('lobby.invitePlayers')}</h3>
...
export default function Home() {
  return (
    <Suspense
      fallback={
        <main className="flex min-h-screen flex-col items-center justify-center p-6 md:p-24 bg-amber-50">
          <h1 className="text-4xl font-bold animate-pulse text-slate-600">Loading Lobby...</h1>
        </main>
      }
    >
      <GameLobby />
      <Toaster
        ...
        toastOptions={{
          className: "!bg-white !text-slate-800 !border !border-amber-200 !font-bold !tracking-wide rounded-xl",
