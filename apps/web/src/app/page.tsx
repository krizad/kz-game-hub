                  ...
                  >
                    <div>
                      <div className="text-slate-800 font-bold tracking-widest text-lg leading-none mb-1 group-hover:text-indigo-300 transition-colors flex items-center gap-2">
                        {r.code}
                        <span
                          className={`text-[9px] px-1.5 py-0.5 rounded border leading-none ml-2 tracking-normal font-sans ${r.gameType === GameType.GOBBLER_TIC_TAC_TOE || r.gameType === GameType.TIC_TAC_TOE ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' : r.gameType === GameType.RPS ? 'bg-amber-500/10 text-amber-400 border-amber-500/20' : 'bg-indigo-500/10 text-indigo-400 border-indigo-500/20'}`}
                        >
                          {r.gameType === GameType.GOBBLER_TIC_TAC_TOE
                            ? t('lobby.gameNames.gobbler').toUpperCase()
                            : r.gameType === GameType.TIC_TAC_TOE
                              ? t('lobby.gameNames.ticTacToe').toUpperCase()
                              : r.gameType === GameType.RPS
                                ? t('lobby.gameNames.handDuel').toUpperCase()
                                : r.gameType === GameType.DETECTIVE_CLUB
                                  ? 'DETECTIVE CLUB'
                    ...
                    <div className="flex items-center gap-3">
                      <div
                        className="flex items-center gap-1.5 text-xs font-bold text-slate-600 bg-amber-50 px-2.5 py-1.5 rounded-lg border border-amber-200 shadow-inner group-hover:border-indigo-500/30 transition-colors"
                ...
                <table className="w-full text-sm text-left relative">
                  <thead className="text-[10px] text-slate-500 uppercase bg-white/90 backdrop-blur-md sticky top-0 border-b border-amber-200/80 shadow-sm">
                    <tr>
                      ...
                      <th className="px-3 py-2 text-right font-bold tracking-wider">
                        {t('lobby.score')}
                      </th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-amber-200/30">
                    {room.players.map((p) => (
                      <tr
                        key={p.id}
                        className="bg-amber-100/10 hover:bg-amber-100/40 transition-colors"
                      >
                            ...
                            {useGameStore.getState().socketId === room.roomHostId ? (
                              <select
                                ...
                                onChange={(e) =>
                                  useGameStore
                                    .getState()
                                    .updateConfig({ hostSelection: e.target.value as any })
                                }
                              ...
                              <div className="text-slate-700 font-medium text-sm px-3 py-2 bg-white/50 rounded-lg border border-amber-200/50">
                                {room.config?.hostSelection === 'ROUND_ROBIN'
                                  ? t('lobby.roundRobin')
                                  : room.config?.hostSelection === 'RANDOM'
                                    ? t('lobby.random')
                                    : t('lobby.roomCreatorFixed')}
                              </div>
                            )}
                          </div>
                          <div>
                            ...
                            {useGameStore.getState().socketId === room.roomHostId ? (
                              <input
                                ...
                                max="10"
                                value={
                                  room.config?.timerMin === undefined ? 5 : room.config?.timerMin
                                }
                                onChange={(e) => {
                                  const val = e.target.value;
                                  useGameStore.getState().updateConfig({
                                    timerMin: val === '' ? ('' as any) : parseInt(val),
                                  });
                                }}
                                onBlur={() => {
                            ...
                            {useGameStore.getState().socketId === room.roomHostId ? (
                              <select
                                ...
                                onChange={(e) => {
                                  useGameStore
                                    .getState()
                                    .updateConfig({ rpsMode: e.target.value as any });
                                }}
                                className="w-full bg-white border border-amber-300 rounded-lg px-3 py-2 text-sm text-slate-800 focus:outline-none focus:border-indigo-500 appearance-none"
                              >
                                <option value="1V1_ROUND_ROBIN">
                            ...
                            {useGameStore.getState().socketId === room.roomHostId ? (
                              <select
                                ...
                                onChange={(e) => {
                                  useGameStore
                                    .getState()
                                    .updateConfig({ rpsBestOf: parseInt(e.target.value) });
                                }}
                                className="w-full bg-white border border-amber-300 rounded-lg px-3 py-2 text-sm text-slate-800 focus:outline-none focus:border-indigo-500 appearance-none"
                              >
                        ...
                        <>
                          <div>
                            ...
                            {useGameStore.getState().socketId === room.roomHostId ? (
                              ...
                              <div className="text-slate-700 font-medium text-sm px-3 py-2 bg-white/50 rounded-lg border border-amber-200/50">
                                {room.config.maxRounds || 3} Rounds
                              </div>
                            )}
                          </div>
                          ...
                          {room.config.wordMode === 'RANDOM' && (
                            <div>
                              ...
                              {useGameStore.getState().socketId === room.roomHostId ? (
                                ...
                                <div className="text-slate-700 font-medium text-sm px-3 py-2 bg-white/50 rounded-lg border border-amber-200/50">
                                  {room.config.wordCategory || 'Not selected'}
                                </div>
                              )}
                            </div>
                          )}
                          ...
                          {room.config.wordMode === 'PLAYER_INPUT' && (
                            <div>
                              ...
                              {useGameStore.getState().socketId === room.roomHostId ? (
                                ...
                                <div className="text-slate-700 font-medium text-sm px-3 py-2 bg-white/50 rounded-lg border border-amber-200/50">
                                  {room.config.wordCategory || 'Any Topic'}
                                </div>
                              )}
                            </div>
                          )}
                  ...
                  .map((p) => (
                    ...
                    >
                      <label className="block text-sm font-bold text-slate-700 mb-1 flex items-center gap-2">
                        <span className="w-6 h-6 rounded-full bg-white flex items-center justify-center text-xs border border-amber-300">
                          {getAvatarEmoji(p.id)}
                        </span>
                        {p.name}
                      </label>
