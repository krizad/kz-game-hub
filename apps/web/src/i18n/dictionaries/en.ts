import { Dictionary } from './schema';

export const en: Dictionary = {
  lobby: {
    connecting: 'Connecting...',
    invited: "You've been invited!",
    joinRoomInfo: 'Join room',
    displayName: 'Display Name',
    enterNamePlaceholder: 'Enter your name to play',
    enterNameShort: 'Enter your name',
    enterGame: 'Enter Game',
    createOwnRoom: 'Or create your own room',
    returnToHome: 'Return to Home',
    gameLobbyTitle: 'GAME LOBBY',
    seconds: 'seconds',
    gameNames: {
      whoKnow: 'Who Know!',
      gobbler: 'Gobbler Tic Tac Toe',
      ticTacToe: 'Classic Tic Tac Toe',
      ultimateTTT: 'Ultimate Tic-Tac-Toe',
      handDuel: 'Hand Duel',
      soundsFishy: 'Sounds Fishy',
      whoAmI: 'Who Am I',
      detectiveClub: 'Detective Club',
      whoFirst: 'Who First',
      theMind: 'The Mind',
      musicTrivia: 'Music Trivia',
      coup: 'Coup',
      saboteur: 'Saboteur',
      pokDeng: 'Pok Deng',
    },
    or: 'OR',
    roomCodePlaceholder: 'ROOM CODE',
    join: 'Join',
    publicLobbies: 'Public Lobbies',
    host: 'Host',
    playersInRoom: 'Players currently in room',
    roomHost: 'Room Host:',
    unknownHost: 'Unknown',
    copyLink: 'Copy Link',
    qrCode: 'QR Code',
    leave: 'Leave Room',
    confirmLeave: 'Confirm Leave',
    players: 'Players',
    score: 'Score',
    offline: 'OFFLINE',
    you: 'YOU',
    viewer: 'VIEWER',
    locked: 'LOCKED',
    waitingRoom: 'Waiting Room',
    hostSelection: 'Host Selection',
    roundRobin: 'Round Robin',
    random: 'Random',
    roomCreatorFixed: 'Room Creator (Fixed)',
    timerMinutes: 'Timer (Minutes)',
    minutes: 'Minutes',
    mode: 'Mode',
    oneVOneRoundRobin: '1v1 Round Robin',
    allAtOnce: 'All At Once',
    targetScore: 'Target Score (Best Of)',
    bo1: 'BO1 (First to 1)',
    bo3: 'BO3 (First to 2)',
    bo5: 'BO5 (First to 3)',
    bo7: 'BO7 (First to 4)',
    bestOf: 'Best of',
    startGame: 'Start Game',
    startingGame: 'Starting...',
    waitingMin: 'Waiting (min {count})',
    waitingForHost: 'Waiting for Room Host to start',
    waitingForSecretWord: 'Waiting for Secret Word...',
    phase: 'Phase',
    secretWordSelection: 'SECRET WORD SELECTION',
    youAreHost: 'You are the Host',
    enterSecretWordDesc:
      'Enter the secret word for this round. Only you and the Insider will know what it is!',
    typeSecretWord: 'Type secret word...',
    confirmSecretWord: 'Confirm Secret Word',
    leaveRoomTitle: 'Leave Room?',
    leaveRoomDesc: "Are you sure you want to leave the room?\nYou'll need the room code to rejoin.",
    cancel: 'Cancel',
    leaveRoomBtn: 'Leave Room',
    invitePlayers: 'Invite Players',
    scanQrCodeDesc: 'Scan this QR code to join room',
    close: 'Close',
  },
  errors: {
    enterNameFirst: 'Please enter your display name first',
    inviteLinkCopied: 'Invite link copied!',
  },
  rules: {
    button: 'Rules',
    modal: {
      title: 'How to Play',
      tabs: {
        overview: 'Overview',
        whoKnow: 'Who Know',
        gobbler: 'Gobbler',
        ticTacToe: 'Tic Tac Toe',
        ultimateTTT: 'Ultimate Tic-Tac-Toe',
        handDuel: 'Hand Duel',
        soundsFishy: 'Sounds Fishy',
        whoAmI: 'Who Am I',
        detectiveClub: 'Detective Club',
        whoFirst: 'Who First',
        musicTrivia: 'Music Trivia',
        theMind: 'The Mind',
        saboteur: 'Saboteur',
        coup: 'Coup',
        pokDeng: 'Pok Deng',
      },
      closeBtn: "Got it, let's play!",
    },
    lobby: {
      welcomeTitle: 'Welcome to KZ Game Hub',
      welcomeDesc:
        "This is a multiplayer party game platform. Create a room or join a friend's room to play various games together!",
      modesTitle: 'Available Game Modes',
      whoKnowDesc:
        'A game of deduction and deception. One player is the Insider trying to guide the group to guess a secret word without being too obvious, while the Commoners try to figure out both the word and who the Insider is!',
      gobblerDesc:
        'A strategic twist on the classic game! Each player has pieces of different sizes that can "gobble" up smaller pieces. You can even move your pieces around the board!',
      ticTacToeDesc: 'The timeless classic. Line up 3 of your marks in a row to win.',
      handDuelDesc:
        'An intense, competitive version of Rock Paper Scissors. Play in 1v1 Round Robin or Chaotic All-at-Once modes!',
      soundsFishyDesc: 'A fast-paced bluffing party game where one player tries to spot the liars.',
      whoAmIDesc:
        'A classic party game where players try to guess the word assigned to them by asking yes/no questions.',
      detectiveClubDesc:
        'A bluffing and image interpretation game. Everyone knows the secret word except the Conspirator, who must blend in.',
      whoFirstDesc:
        'The fastest reactor wins! Compete to press the button first when the signal drops.',
      musicTriviaDesc:
        'Test your musical knowledge! Be the fastest to guess the song correctly to win.',
      theMindDesc:
        'An innovative cooperative card game where you must play cards in ascending order without communicating at all!',
      saboteurDesc: 'A hidden-role mining game. Miners dig to gold, Saboteurs sabotage secretly.',
      coupDesc:
        'A bluffing deduction game — lie about your influence, block and assassinate to be the last one standing.',
    },
    coup: {
      title: 'Coup',
      desc: 'Bluff your influence to collect coins and eliminate rivals.',
      setupTitle: 'Setup',
      setupDesc: 'Shuffle 15 cards, deal 2 per player + 2 coins. Look at your own cards anytime.',
      rolesTitle: 'Roles',
      roleDuke: 'Duke — Tax +3, Blocks Foreign Aid',
      roleAssassin: 'Assassin — Pay 3 to Assassinate 1',
      roleCaptain: 'Captain — Steal 2, Blocks Steal',
      roleAmbassador: 'Ambassador — Exchange 2, Blocks Steal',
      roleContessa: 'Contessa — Blocks Assassination',
      flowTitle: 'Flow',
      flowDesc: 'Declare → Challenge 7s → Block 7s → Resolve',
    },
    whoKnow: {
      setupTitle: 'The Setup',
      setupDesc1: '"WHO KNOW?" is a game of deduction and deception. One player is the ',
      setupDescGameHost: 'Game Host',
      setupDesc2: ', one is the secret ',
      setupDescInsider: 'Insider (Know)',
      setupDesc3: ', and the rest are ',
      setupDescCommoners: 'Commoners (Unknow)',
      phasesTitle: 'The Phases',
      phase1Title: 'Word Setting',
      phase1Desc:
        'The Game Host picks a secret word. Only the Host and the Insider know what it is.',
      phase2Title: 'Questioning',
      phase2Desc:
        'Everyone has 3 minutes to ask the Host "Yes" or "No" questions to guess the word. The Insider secretly guides the group towards the answer without being completely obvious.',
      phase3Title: 'Voting',
      phase3Desc:
        'If the word is guessed in time, everyone votes on who they think the Insider was.',
      winCondTitle: 'Winning Conditions',
      winCond1A: 'If the group ',
      winCond1B: 'fails',
      winCond1C: ' to guess the word in 3 minutes → Everyone loses.',
      winCond2A: 'If the group guesses the word, but ',
      winCond2B: 'fails',
      winCond2C: ' to catch the Insider → The Insider wins!',
      winCond3A: 'If the group guesses the word AND correctly votes out the Insider → ',
      winCond3B: 'The Commoners win!',
    },
    gobbler: {
      title: 'Gobbler Tic Tac Toe',
      desc: 'A strategic twist on the classic game! Each player has pieces of different sizes that can "gobble" up smaller pieces.',
      rulesTitle: 'The Rules',
      pieceSizesTitle: 'Piece Sizes',
      pieceSizesDesc:
        'You have pieces in 3 sizes: Small, Medium, and Large. Each player gets 2 of each size.',
      gobblingTitle: 'Gobbling',
      gobblingDesc1: 'You can place a larger piece ',
      gobblingDesc2: 'over any smaller piece',
      gobblingDesc3:
        ' already on the board (even your own!). The smaller piece gets hidden and loses its spot unless the bigger piece is moved later.',
      movingTitle: 'Moving Pieces',
      movingDesc:
        "Instead of playing a new piece from your inventory, you can move one of your pieces already on the board to a new spot (even to gobble another piece). But be careful not to reveal an opponent's piece that might complete their line!",
      winningTitle: 'Winning',
      winningDesc:
        'First to line up 3 of their visible pieces in a row horizontally, vertically, or diagonally wins!',
    },
    ticTacToe: {
      title: 'Classic Tic Tac Toe',
      desc: "The classic game of X's and O's. Two players take turns placing their mark on a 3x3 grid.",
      winTitle: 'How to Win',
      winDesc1A: 'The first player to get ',
      winDesc1B: '3 of their marks in a row',
      winDesc1C: ' (horizontally, vertically, or diagonally) wins the game.',
      winDesc2A: 'If all 9 squares are filled and neither player has 3 in a row, the game is a ',
      winDesc2B: 'Draw',
    },
    ultimateTTT: {
      title: 'Ultimate Tic-Tac-Toe (Super Tic-Tac-Toe)',
      desc: 'An advanced strategic evolution of Tic-Tac-Toe played on a nested 3x3 grid of 9 small boards (81 squares total). Force your opponent across the board to claim macro squares!',
      rulesTitle: '4 Core Rules',
      rule1Title: 'Nested Boards',
      rule1Desc:
        'There is a large 3x3 macro board, and within each of its 9 squares is a smaller 3x3 micro board (81 squares total).',
      rule2Title: 'Turn Forwarding Mechanism',
      rule2Desc:
        'The cell you choose within a small board dictates where your opponent MUST play on the large board next. For example, playing in the top-right corner of a small board sends your opponent to the top-right board on the macro grid!',
      rule3Title: 'Claiming Territory',
      rule3Desc:
        'Whoever gets 3 in a row on a small board claims that entire macro square as their symbol (X or O).',
      rule4Title: 'Free Move',
      rule4Desc:
        'If you are sent to a small board that is already won or completely full, you earn a Free Move to play in ANY open cell on any unfinished board!',
      winTitle: 'Winning Condition',
      winDesc:
        'Win the game immediately by aligning 3 claimed macro boards in a row (horizontally, vertically, or diagonally)!',
      diagramTitle: 'Turn Forwarding Diagram Example',
      diagramDesc:
        'If you play in the top-right cell (↗) of the center sub-board → Your opponent is forced to play in the top-right macro board!',
      diagramTargetNotice: 'Target Board Forced to Play',
    },
    rps: {
      title: 'Hand Duel (Rock Paper Scissors)',
      desc: 'An intense, competitive version of the timeless hand game. Outsmart your opponents to claim victory.',
      basicsTitle: 'The Basics',
      beats: 'Beats',
      modesTitle: 'Game Modes',
      mode1v1Title: '1v1 Round Robin',
      mode1v1Desc:
        'Players face off in a series of 1-on-1 duels until everyone has battled each other. Winning a duel gives you a point.',
      modeAllTitle: 'All At Once',
      modeAllDesc:
        "Chaos reigns! All players throw at the same time. The winning sign is the one that beats the most other signs. If there's a tie, nobody scores. First to reach the target score wins!",
    },
    soundsFishy: {
      title: 'Sounds Fishy',
      desc: 'Fast-paced bluffing party game where everyone knows the real answer... except for one person. Can you spot the truth from the lies?',
      rolesTitle: 'Roles',
      rolePicker: 'The Picker',
      rolePickerDesc:
        'Your job is to ask questions and figure out who is telling the truth (Blue Fish) and who is lying (Red Herrings).',
      roleBlueFish: 'Blue Fish',
      roleBlueFishDesc:
        'You know the true answer to the question. You MUST provide the exact true answer.',
      roleRedHerring: 'Red Herring',
      roleRedHerringDesc: 'You see the real answer, but you MUST provide a believable fake answer.',
      flowTitle: 'Game Flow',
      flowSetup: 'The game assigns one Picker and one Blue Fish. The rest are Red Herrings.',
      flowPitch: "The Picker reveals players' answers one by one.",
      flowHunt:
        'After all answers are revealed, the Picker can choose to eliminate a player who looks fishy (a Red Herring) or bank the points.',
      flowScore:
        'If the Picker eliminates the Blue Fish, they lose all points for the round, and the Blue Fish + surviving Red Herrings score.',
      scoringTitle: 'Scoring System',
      scoreRedHerring: 'Catch a Red Herring: +1 Point into the temporary pool.',
      scoreBank: 'Bank Points: End your turn immediately and secure the points in the pool.',
      scoreBlueFish:
        'Catch the Blue Fish: You get 0 points. Blue Fish gets +1 for every surviving Red Herring. Each surviving Red Herring gets +1.',
    },
    whoAmI: {
      title: 'Who Am I',
      desc: 'A classic party game where players try to guess the word assigned to them by asking yes/no questions.',
      setupTitle: 'Setup',
      setupDesc:
        "Every player writes a word and submits it. Words are shuffled and assigned to each player. You can see everyone's word except your own!",
      gameplayTitle: 'Gameplay',
      gameplayDesc:
        'Players take turns asking a yes/no question out loud to the group to figure out their word. The group votes Yes, No, or Maybe.',
      guessingTitle: 'Guessing',
      guessingDesc:
        "You can make one guess during your turn. If you guess right, you win that round! If you're wrong, you continue to play but lose your ability to guess again.",
      endgameTitle: 'Endgame',
      endgameDesc:
        'The game ends when everyone guesses their word or the game reaches the final round.',
    },
    detectiveClub: {
      title: 'Detective Club',
      desc: 'A bluffing + Dixit game where players interpret abstract card images. Everyone knows the secret word... except the Conspirator who must bluff.',
      rolesTitle: 'Roles',
      informerDesc:
        "Informer — Chooses a word from their hand and writes it in the notebooks, except the Conspirator's notebook is blank. Always plays the first card.",
      conspiratorDesc:
        "Conspirator — Doesn't know the word! Must play cards to blend in without getting caught.",
      detectiveDesc:
        'Detective — Knows the word, plays cards normally, and tries to figure out who the Conspirator is.',
      stepsTitle: 'Game Steps',
      step1Setup:
        'Setup — Roles are assigned → Informer chooses a word → Everyone sees the word except the Conspirator.',
      step2Playing:
        'Card Playing (2 rounds) — Each player selects cards from hand. Informer starts first, then clockwise.',
      step3Discussion:
        'Discussion — Informer explains why their cards match the word first, then everyone takes turns explaining.',
      step4Voting:
        'Voting — Everyone (except the Informer) votes on who they think the Conspirator is.',
      step5Scoring:
        'Scoring — Results are revealed. Points are awarded based on whether the Conspirator was caught.',
      scoringTitle: 'Scoring',
      caughtScoring: 'Conspirator caught: Detectives who guessed right get +3, Informer gets +3.',
      escapedScoring: 'Conspirator escaped: Conspirator gets +5, Informer gets 0.',
    },
    whoFirst: {
      title: 'Who First',
      desc: 'Compete to see who has the fastest reaction time!',
    },
    musicTrivia: {
      title: '🎵 Music Trivia',
      desc: 'Guess the song and artist as fast as you can. Fastest fingers win!',
      rules: {
        sharedFeaturesTitle: '🌟 Shared Features (Both Modes)',
        sharedFeatures:
          "Audio Playback: Host can choose to play music on all devices, or only on the host's device.||Manual Start: The host manually starts each song, followed by a countdown before playback.||Audio Permission: If playing on all devices, players must ensure they have interacted with the page to allow audio playback.||Fastest Finger: Only the first person to press the Buzzer gets to answer.||Global Pause: When someone buzzes, the music pauses.||Strike Out: 1 wrong answer = Struck out for the current song. The music resumes for the rest to guess.",
        modeTypingTitle: '⌨️ Typing Mode',
        modeTypingDesc: 'Focus on precision. Everyone including the host can play.',
        modeTypingRules:
          'The buzzed player must "type" the correct answer before the configured time runs out.||Answer Criteria: The host can configure whether you need to guess the Song Title, the Artist, or either one.||Fuzzy Search Engine: Minor typos or missing spaces are forgiven!||Correct answers automatically earn 1 point.',
        modeGmTitle: '👑 Game Master Mode',
        modeGmDesc: 'Fast-paced gameplay. No input box for players.',
        modeGmRules:
          'The host cannot play. They act as the "Judge" to listen to answers.||When buzzed, the screen says "Say your answer out loud!" The player must shout the answer.||The host decides if it is correct by clicking ✅ Yes or ❌ No on their screen.||If no one knows: The host can click the Reveal button to show the answer and skip to the next song.',
        tipsTitle: '💡 Additional Tips',
        tips: 'Speed is everything! Keep your finger ready on the buzzer.||If a player buzzes but answers wrong, get ready! The music will resume immediately.',
      },
    },
  },
  gameSoundsFishy: {
    loading: 'Loading game state...',
    yourRole: 'Your Role',
    rolePicker: 'The Picker',
    roleBlueFish: 'Blue Fish (Truth)',
    roleRedHerring: 'Red Herring (Lie)',
    currentPot: 'Current Pot',
    pts: 'PTS',
    setupPhase: 'Setup Phase',
    theTopic: 'The Topic',
    trueAnswer: 'The True Answer',
    waitingForFish: 'Waiting for fish to write their answers...',
    answerSubmitted: 'Answer Submitted!',
    waitingForOthers: 'Waiting for others...',
    mustEnterTrue: 'You MUST enter the true answer exactly.',
    mustEnterFake: "You MUST enter a believable fake answer. Don't use the exact truth!",
    typeAnswerPlaceholder: 'Type your answer here...',
    submitAnswer: 'Submit Answer',
    otherFishesTyping: 'Other Fishes are typing...',
    revealEveryoneFirst: "Reveal EVERYONE's answer first to start the hunt!",
    allRevealedEliminate: 'All answers revealed! Now eliminate the Red Herrings.',
    revealAnswer: 'Reveal Answer',
    eliminateLooksFishy: 'Eliminate (Looks Fishy)',
    eliminated: 'Eliminated',
    youHavePointsPrefix: 'You have ',
    youHavePointsSuffix:
      ' points in the pot. You can bank them now and end the round, or keep hunting!',
    bankPointsAndEnd: 'Bank Points & End Round',
    roundOver: 'Round Over!',
    howEveryoneScored: "Here's how everyone scored:",
    pickerText: 'Picker',
    blueFishText: 'Blue Fish',
    redHerringText: 'Red Herring',
    plusPointsPrefix: '+',
    plusPointsSuffix: ' This Round',
    playNextRound: 'Play Next Round',
  },
  gameWhoAmI: {
    lobby: {
      numRounds: 'Number of Rounds',
      roundsCount: '{count} Rounds',
      wordMode: 'Word Mode',
      modeHostPick: '📝 Host Picks',
      modeRandom: '🎲 Random (DB)',
      modePlayer: '✍️ Players Write',
      modeAi: '🤖 AI Generate',
      category: 'Category',
      noCategories: 'No categories found.',
      selectCategory: 'Select a category...',
      notSelected: 'Not selected',
      themeOptional: 'Theme (Optional)',
      themePlaceholder: 'e.g. Animals, Movies...',
      anyTopic: 'Any Topic',
      aiPrompt: 'AI Prompt / Topic',
      aiPromptPlaceholder: 'e.g. Ghost, Strange Food...',
      randomTopic: 'Random Topic',
    },
    submitYourWord: 'Submit Your Word',
    category: 'Category',
    wordSubmissionDesc:
      "Each player writes a word. Words are shuffled so you won't get your own! Duplicates will be rejected.",
    wordSubmitted: 'Word submitted!',
    waitingForOthers: 'Waiting for others...',
    typeYourWord: 'Type your word...',
    submitWord: 'Submit Word',
    submissions: 'Submissions',
    finalGuessRound: 'Final Guess Round',
    round: 'Round',
    spectating: 'You are spectating. Current turn:',
    yourTurn: "It's Your Turn!",
    waitingFor: 'Waiting for',
    toPlay: 'to play...',
    eliminated: 'Eliminated',
    active: 'Active',
    theyAre: 'They are',
    finalGuessDescActive: 'This is your last chance! Guess your word now!',
    finalGuessDescWait: 'Waiting for the player to guess their word...',
    askingPhase: 'Asking Phase',
    askingPhaseDescActive: 'Ask the group a Yes/No question out loud!',
    askingPhaseDescWait: "Vote on the player's verbal question!",
    endTurnHint: 'When you are satisfied with the answers, click below to end your turn.',
    waitingForVotes: 'Waiting for votes...',
    majorityAnswer: 'Majority Answer',
    yes: 'YES',
    no: 'NO',
    maybe: 'MAYBE',
    endAsking: 'End Asking',
    usedGuess: "You've used your guess",
    guessTheWord: 'Guess the Word!',
    unknown: 'Unknown',
    castVoteHint: 'Cast your vote based on their word (you can change it until they end the turn):',
    voteCastHint: 'Vote cast! The asker can see your vote.',
    waitingForPlayersToVote: 'Waiting for players to vote...',
    wordGuess: 'Word Guess',
    guesses: 'guesses:',
    isGuessCorrect: 'Is their guess correct?',
    voteCast: 'Vote cast!',
    waitingForVerify: 'Waiting for other players to verify your guess...',
    continueBtn: 'Continue',
    gameOver: 'Game Over',
    noWinners: 'No winners this round',
    theWordsWere: 'The Words Were',
    wins: 'WINS!',
    theyGuessedCorrectly: 'They guessed their word correctly.',
    noWinner: 'NO WINNER',
    everyoneUsedGuesses: 'Everyone used all their guesses.',
    whoAreYou: 'Who Are You?',
    takeAGuessDesc:
      "Take a guess! If you're wrong, you might be eliminated or lose your chance to win.",
    typeYourGuess: 'Type your guess...',
    cancel: 'Cancel',
    submitGuess: 'Submit Guess',
  },
  gameDetectiveClub: {
    loading: 'Loading Detective Club...',
    yourRole: 'Your Role',
    yourScore: 'Your Score',
    pts: 'pts',
    informer: 'Informer',
    conspirator: 'Conspirator',
    detective: 'Detective',
    unknownRole: 'Unknown',
    setupPhase: 'Setup Phase',
    informerSetup:
      'You are the Informer! Look at your hand and pick a word that connects your cards.',
    waitingInformer: 'Waiting for Informer to pick a word...',
    yourHandCards: 'Your Hand Cards',
    typeWordConnectedToCards: 'Type a word that connects to your hand cards',
    wordPlaceholder: 'e.g. Space, Ocean, Magic...',
    confirmWord: 'Confirm Word',
    waitingForInformer: 'Waiting for Informer to choose a word...',
    theSecretWord: 'The Secret Word',
    roundOf: 'Round {current} of {total}',
    playedCards: 'Played Cards',
    yourTurnPlayCard: 'Your Turn - Play a Card',
    waitingFor: 'Waiting for {name}...',
    yourHand: 'Your Hand',
    play: 'Play',
    confirmCardPlay: 'Confirm Card Play',
    confirmPlayDescription: 'Are you sure you want to play this card?',
    cancel: 'Cancel',
    playCard: 'Play Card',
    discussionPhase: 'Discussion Phase',
    informerExplain: 'The Informer must now explain why their cards match the word:',
    everyoneExplain:
      'Then, everyone else takes turns explaining their cards. The Conspirator must bluff!',
    startVoting: 'Start Voting',
    waitingForHostVoting: 'Waiting for the host to start the voting phase...',
    votingPhase: 'Voting Phase',
    detectivesVoting: 'Detectives are voting on who they think the Conspirator is.',
    informerSitTight: 'You are the Informer, sit tight!',
    whoIsConspirator: 'Who is the Conspirator?',
    reviewCardsAndVote:
      'Review the played cards and vote for the player you think did NOT know the word:',
    voteLockedIn: 'Vote Locked In',
    waitingForOtherPlayers: 'Waiting for other players...',
    confirmVote: 'Confirm Vote',
    roundOver: 'Round Over',
    conspiratorWas: 'The Conspirator was:',
    conspiratorCaught: 'Conspirator Caught!',
    detectivesIdentified: '{count} Detectives correctly identified them.',
    conspiratorEscaped: 'Conspirator Escaped!',
    nobodyCaught: 'Nobody caught the Conspirator! They fooled you all.',
    scoreboard: 'Scoreboard',
    votedFor: 'Voted for:',
    playNextRound: 'Play Next Round',
    endGame: 'End Game',
    waitingForHostNextRound: 'Waiting for the host to start the next round...',
  },
  gameTicTacToe: {
    lobby: 'Tic-Tac-Toe Lobby',
    joinAs: 'Join as {side}',
    waitingJoin: 'Waiting for both players to join...',
    playing: 'Playing',
    gameOver: 'Game Over',
    score: 'Score',
    draw: "It's a Draw!",
    wins: '{winner} Wins!',
    playAgain: 'Play Again',
    modes: {
      title: 'Game Mode',
      classic: 'Classic (3x3)',
      classicDesc: 'Traditional Tic-Tac-Toe, line up 3 to win',
      gobbler: 'Gobbler',
      gobblerDesc: 'Bigger pieces devour smaller ones on a 3x3 board',
      ultimate: 'Ultimate (9x9)',
      ultimateDesc: 'Nested 9x9 grid, win sub-boards to claim the main grid',
      hostSelectingMode: 'Host is selecting game mode...',
      changeMode: 'Change Mode / Return to Lobby',
    },
    bot: {
      title: 'Opponent',
      pvp: 'Play with Friend (PvP)',
      pvpDesc: 'Play against other players in the room',
      vsBot: 'Play vs Bot',
      vsBotDesc: 'Challenge an AI opponent',
      difficulty: 'Bot Difficulty',
      easy: 'Easy Mode',
      easyDesc: 'Bot places moves randomly, casual play',
      god: 'God Mode',
      godDesc: 'Bot calculates optimal moves, mathematically unbeatable!',
      thinking: 'Bot is thinking...',
      botBadge: 'Bot 🤖',
      playVsBotAs: 'Choose your side to start vs Bot',
      hostSelectingBot: 'Host is configuring bot settings...',
    },
  },
  gameUltimateTTT: {
    title: 'Ultimate Tic-Tac-Toe',
    yourTurn: 'Your Turn!',
    opponentTurn: "{turn}'s Turn",
    freeMove: '🌟 Free Move: Play on any open board!',
    targetBoard: '🎯 Board #{board}',
  },
  gameGobbler: {
    lobby: 'Gobbler',
    gobblerTagline: 'Larger pieces can gobble smaller ones! Select your team to begin.',
    join: 'Join {side}',
    inventory: '{side} Inventory',
    team: 'Team {side}',
    yourTurn: 'Your Turn',
    turn: "{side}'s Turn",
    draw: 'Draw!',
    drawSubtitle: 'A truly balanced match.',
    wins: '{winner} Wins!',
    claimsVictory: '{name} claims victory!',
    playAgain: 'Play Again',
    player: 'Player {side}',
  },
  gameSlave: {
    title: '⛓️ Slave',
    leaderLine: 'Leader: {name}',
    trick: 'Trick',
    noTrick: 'Waiting for the lead…',
    yourHand: 'Your hand',
    play: 'Play',
    pass: 'Pass',
    waiting: 'Waiting for other players…',
    resultLine: 'Winners: {winners}',
  },
  gameSamSip: {
    title: '🀄 Sam Sip',
    discard: 'Latest discard',
    noDiscard: 'No discard yet',
    yourHand: 'Your hand',
    draw: 'Draw',
    claim: 'Claim',
    pickDiscard: 'Pick a card to discard',
    waiting: 'Waiting for other players…',
    resultLine: 'Winners: {winners}',
  },
  gameOldMaid: {
    title: '🃏 Old Maid',
    yourHand: 'Your hand',
    takeHint: 'Pick one face-down card from {name}',
    waiting: 'Waiting for other players…',
    loserLine: 'Loser: {name}',
  },
  cardGameLog: {
    title: 'Action log',
    kinds: {
      DREW: '{actor} drew a card',
      STOOD: '{actor} stood',
      PLAYED: '{actor} played {count} card(s)',
      PASSED: '{actor} passed',
      CLAIMED: '{actor} claimed the discard',
      DISCARDED: '{actor} discarded',
      TOOK: '{actor} took a card',
    },
  },
  gamePokDeng: {
    title: '🃏 Pok Deng',
    dealerLine: 'Dealer: {name} · starting chips 100',
    dealerBadge: 'dealer',
    cards: 'cards',
    chips: 'chips',
    yourHand: 'Your hand',
    resultLine: 'Dealer scored {score} · Winners: {winners}',
    dealerWins: 'Dealer',
    nextRound: 'Next round',
    draw: 'Draw third card',
    stand: 'Stand',
    waiting: 'Waiting for players to draw or stand…',
    revealing: 'Revealing the dealer hand…',
  },
  cardGameRules: {
    title: 'Advanced Rules',
    preset: 'Preset',
    noConfig: 'Advanced rules appear once you are in a Pok Deng room.',
    deck: 'Deck',
    deckValue: 'Standard 52 cards ×{copies}',
    dealing: 'Dealing',
    dealValue: '{cards} cards each · {mode}',
    starter: 'First dealer',
    actions: 'Turn actions',
    turnTimer: '{seconds}s timeout → auto {auto}',
    piles: 'Stock',
    reserveValue: 'Reserve of {size} cards',
    visibility: 'Visibility',
    showHandsAtEnd: 'All hands revealed at round end',
    hideHandsAtEnd: 'Hands stay hidden at round end',
    showHandCounts: 'Others card counts visible',
    hideHandCounts: 'Others card counts hidden',
    scoring: 'Chips & scoring',
    chipsValue: 'Start {chips} chips · base stake {stake}',
    tieLine: 'Ties: {policy}',
    multipliers: 'Multipliers',
    mod10Note: 'Hands score mod 10: A = 1, face cards (J/Q/K) = 0.',
    dealModes: {
      EQUAL_WITH_LEFTOVERS: 'leftovers to the stock',
      DEAL_ALL: 'deal every card',
      DEAL_ALL_UNEVEN: 'deal every card (remainder to the earliest seats)',
      REJECT_IF_NOT_EVEN: 'reject unless fully consumed',
    },
    starterPolicies: {
      RANDOM: 'Random',
      HOST_SELECT: 'Host selects',
      ROTATE: 'Rotate in seat order',
      PREVIOUS_WINNER: 'Previous winner',
      PREVIOUS_LOSER: 'Previous loser',
    },
    actionNames: { DRAW: 'Draw', STAND: 'Stand' },
    exhaustionPolicies: {
      END_ROUND: 'End the round',
      RESHUFFLE_DISCARDS_EXCEPT_TOP: 'Reshuffle discards (keep top card)',
      USE_RESERVE: 'Use the reserve',
    },
    tiePolicies: {
      DEALER_WINS: 'dealer wins',
      PLAYER_WINS: 'player wins',
      PUSH: 'push (no transfer)',
    },
    outcomes: {
      THREE_OF_A_KIND: 'Three of a kind',
      THREE_FACE: 'Three face cards',
      STRAIGHT_FLUSH: 'Straight flush',
      POK_9: 'Pok 9',
      POK_8: 'Pok 8',
      NORMAL: 'Normal',
    },
  },
  cardGameSettings: {
    title: 'Advanced Rule Settings',
    hostOnly: 'Only the host can change these settings',
    preset: 'Game preset',
    presets: {
      POK_DENG: 'Pok Deng',
      SLAVE: 'Slave',
      SAM_SIP: 'Sam Sip',
      OLD_MAID: 'Old Maid',
    },
    starter: 'First dealer',
    tiePolicy: 'Tie handling',
    turnTimer: 'Turn timer',
    timerOff: 'Off',
    timerSeconds: '{seconds}s',
    preview: '{cards} cards each · {stock} left in the stock',
    previewInvalid: 'This card count cannot be dealt evenly',
    publish: 'Publish rules',
    publishing: 'Publishing…',
    published: 'Share code: {code}',
    copy: 'Copy',
    copied: 'Copied',
    import: 'Import rules by code',
    importPlaceholder: 'Enter share code',
    importButton: 'Import',
    importing: 'Importing…',
    copyOnEdit: 'Editing rules? Publish again to mint a fresh code.',
    errors: {
      INVALID_CONFIG: 'These rules are not valid for this preset',
      NOT_FOUND: 'No rules found for that code',
      PRESET_MISMATCH: 'That code belongs to a different game',
      INVALID_STORED_CONFIG: 'The stored rules failed validation',
      STORAGE_ERROR: 'Storage failed. Please try again.',
      SHARE_CODE_EXHAUSTED: 'Could not allocate a code. Please try again.',
      INVALID_ROOM: 'This room is not a Pok Deng room',
      NOT_HOST: 'Only the room host can do that',
      NOT_LOBBY: 'Rules can only change in the lobby',
    },
  },
  gameRps: {
    playing: 'Playing',
    matchOver: 'Match Over',
    roundOver: 'Round Over',
    allAtOnce: 'ALL AT ONCE',
    oneVOneRoundRobin: '1V1 ROUND ROBIN',
    firstTo: 'FIRST TO {score}',
    vs: 'VS',
    spectating: 'Spectating...',
    choiceLocked: 'Choice locked!',
    waitingOpponent: 'Waiting for opponent...',
    draw: "It's a Draw!",
    winsMatch: '{winner} Wins the Match!',
    winsRound: '{winner} Wins the Round!',
    playAgain: 'Play Again',
    nextRound: 'Next Round',
    queue: 'Queue',
    rock: 'Rock',
    paper: 'Paper',
    scissors: 'Scissors',
  },
  result: {
    winners: 'Winners',
    playAgain: 'Play Again',
  },
  whoFirst: {
    lobby: {
      title: 'Who First Lobby',
      penaltyLabel: 'Enable Penalty for Early Press',
      hostPlaysLabel: 'Host Plays',
      minCountdown: 'Min Countdown (s)',
      maxCountdown: 'Max Countdown (s)',
      infiniteRounds: 'Infinite Rounds',
      roundsCount: 'Number of Rounds',
      showCounter: 'Show Timer',
      startBtn: 'Start Countdown!',
      waitingForHost: 'Waiting for host to start the game',
    },
    game: {
      ready: 'Ready...',
      go: 'GO!!',
      pressBtn: 'PRESS!',
      pressed: 'PRESSED!',
      penalty: 'FOUL!',
      spectating: 'Spectating other players',
      endGameNow: 'End Game Now',
    },
    result: {
      roundTitle: 'Round Result',
      finalTitle: 'Final Result',
      earlyPress: 'Pressed Early!',
      noPress: 'No Press',
      nextRoundBtn: 'Next Round',
      endGameBtn: 'End Game',
      allFouls: 'Everyone Fouled!',
    },
    rules: {
      title: 'Rules',
      desc1: 'This is a reaction game. The fastest player to press the button wins.',
      rule1Title: 'Get Ready',
      rule1Desc: 'Wait for the countdown to display "GO!!" before pressing the button.',
      rule2Title: 'Early Press',
      rule2Desc:
        'If the host enables penalties, pressing before the signal will result in a foul for that round.',
      rule3Title: 'Timer Reset',
      rule3Desc:
        'When either side presses the button, the timer resets to the max countdown for the other side.',
      footer:
        'The game ends when a team reaches the set number of fouls. The side with fewer fouls wins!',
    },
  },
  gameMusicTrivia: {
    lobby: {
      musicCategory: 'Music Category / Artist',
      waitingForHost: 'Waiting for Host...',
      region: 'Region',
      regionIntl: '🌐 International',
      regionTh: '🇹🇭 Thai (TH)',
      searchBy: 'Search By',
      searchAnything: '🎯 Anything (All)',
      searchArtist: '🎤 Artist Only',
      searchSong: '🎵 Song Name Only',
      searchAlbum: '💿 Album Only',
      musicSource: 'Music Source',
      sourceItunes: 'Apple Music (iTunes)',
      sourceYoutube: 'YouTube',
      sourceDeezer: 'Deezer',
      sourceSoundcloud: 'SoundCloud',
      sourceSpotify: 'Spotify',
      gameMode: 'Game Mode',
      modeTyping: '⌨️ Typing (Auto Judge)',
      modeVoice: '🎤 Voice (Host Judge)',
      numRounds: 'Number of Rounds',
      roundsCount: '{count} Rounds',
      queryPlaceholder: 'e.g. Taylor Swift, Anime OST, 90s Pop...',
      audioPlayback: 'Audio Playback',
      playbackHostOnly: 'Host Only',
      playbackEveryone: 'Everyone',
      answerType: 'Answer Criteria',
      answerAny: 'Song or Artist (Any)',
      answerTitle: 'Song Name Only',
      answerArtist: 'Artist Name Only',
      yearStart: 'Start Year',
      yearEnd: 'End Year',
      answerTimeout: 'Answer Timeout (seconds)',
      searchCriteria: 'Search Criteria',
      searchKeywords: 'Search Keywords',
      searchPlaceholder: 'e.g. Taylor Swift, Pop 2023',
      searchHint: 'Leave blank to get random songs from the category',
      anyTopic: 'Any Topic',
      gameRules: 'Game Rules',
      playbackHostHint: 'Host plays audio on their device (Discord/Zoom)',
      playbackEveryoneHint: "Audio plays automatically on everyone's device",
      answerCriteria: 'Answer Criteria',
      sourceProvidesVideo: 'Provides Music Video',
      sourceProvidesPreview: 'Provides 30s Preview',
      sourceLabel: 'Music Source',
      regionLabel: 'Region',
    },
    game: {
      viewOnAppleMusic: '🔗 View on Apple Music',
      listenOnAppleMusic: '🔗 Listen on Apple Music',
      roundLabel: 'Round',
      endGame: 'End Game',
      playlist: 'Playlist:',
      regionLabel: 'Region:',
      searchByLabel: 'Search By:',
      yearsLabel: 'Years:',
      modeLabel: 'Mode:',
      modeVoice: '🎤 Voice',
      modeTyping: '⌨️ Typing',
      preparingMusic: 'Preparing Music...',
      searchingFor: 'Searching for',
      on: 'on',
      getReadyTitle: 'Ready for the next song?',
      getReadyHostWait: "You're ready! Waiting for the host to start...",
      getReadyUnlock: 'Tap the button below to unlock audio and get ready!',
      getReadyBtn: "I'm Ready!",
      getReadyPlayers: 'Players Ready:',
      getReadyStartBtn: 'Start Song (Countdown)',
      countdownTitle: 'Get Ready!',
      nowPlaying: 'Now Playing...',
      listenCarefully: 'Listen carefully!',
      cantHearMusic: "🔊 Click here if you can't hear music",
      hostCannotBuzz: 'You are the host. You cannot buzz.',
      skipQuestion: '⏭️ Skip Question',
      buzzed: 'Buzzed!',
      reaction: 'Reaction:',
      isAnswerCorrect: 'Is their answer correct?',
      actualAnswer: 'Actual Answer:',
      by: 'by',
      yesCorrect: 'Yes (Correct)',
      noWrong: 'No (Wrong)',
      sayAnswerOutLoud: 'Say your answer out loud!',
      hostWillJudge: 'The host will judge if you are correct.',
      waitingForHostToJudge: 'Waiting for host to judge...',
      whatIsSongOrArtist: 'What is the song or artist?',
      typeAnswerHere: 'Type answer here...',
      submitAnswer: 'Submit Answer',
      giveUp: 'Give Up',
      waitingForTheirAnswer: 'Waiting for their answer...',
      correct: 'Correct!',
      incorrect: 'Incorrect!',
      guessedRight: 'Guessed Right',
      guessedWrong: 'Guessed Wrong',
      playerAnswered: 'Player answered:',
      theAnswerWas: 'The Answer Was:',
      nextRound: 'Next Round 🎵',
      autoProceedingIn: 'Auto-proceeding in {count}s...',
      wonTheRound: '{name} won the round! (+1 pt)',
      noOneGotItRight: 'No one got it right!',
      gameOver: 'Game Over!',
      returnToLobby: 'Return to Lobby',
      playedSongs: 'Played Songs',
      noWinner: 'No winner',
    },
  },
  gameTheMind: {
    lobby: {
      title: 'The Mind - Ready to Sync?',
      startingLives: 'Starting Lives',
      startingShurikens: 'Starting Shurikens',
      blindMode: 'Blind Mode',
      gameMode: 'Game Mode',
      modeNormal: 'Normal (Classic)',
      modeExtreme: 'Extreme (2 Piles)',
      timeAttack: 'Time Attack',
      maxLevel: 'Max Level (Rounds)',
      readyToStart: 'Ready to start game?',
    },
    game: {
      level: 'Level',
      cardsDealt: '{count} cards dealt to each player',
      readyBtn: 'Ready',
      ready: 'ready',
      waitingForOthers: 'Waiting for others to be ready...',
      pileTop: 'Pile Top',
      whitePileUp: 'White Pile (UP)',
      redPileDown: 'Red Pile (DOWN)',
      playCard: 'Play {card}',
      timesUp: "TIME'S UP!",
      yourHand: 'Your Hand',
      blindModeActive: 'Blind Mode Active! Cards are hidden.',
      useShuriken: 'Use Shuriken',
      exitGame: 'Exit Game',
      shurikenVoteTitle: 'Shuriken Proposal',
      youProposedShuriken:
        'You proposed using a Shuriken to discard the lowest card from each player.',
      shurikenProposedBy:
        '{name} proposed using a Shuriken. Each player will discard their lowest card.',
      shurikenVoteDesc: 'Everyone must agree for the Shuriken to be used.',
      shurikenResultTitle: 'Shuriken Result',
      shurikenResultDesc: 'Cards discarded by the Shuriken',
      playedBy: 'Played by: {name}',
      playedCardsLog: 'Played Cards Log',
      agree: 'Agree',
      mistakeBy: 'Mistake by {name} who played',
      remainingCards: 'Remaining Cards in Hand',
      disagree: 'Disagree',
      votedAgree: 'You agreed to use the Shuriken',
      votedDisagree: 'You disagreed. Shuriken will not be used.',
      waitingForVotes: 'Waiting for all votes...',
      cancelProposal: 'Cancel Proposal',
      levelComplete: 'Level Complete!',
      levelCleared: 'All cards have been played! Moving to the next level.',
      mistake: 'Mistake!',
      livesRemaining: 'Lives Remaining: {lives}',
      discardedCards: 'Discarded cards (lower than played card)',
      nextLevel: 'Next Level',
      resumeLevel: 'Continue',
      youWin: 'You Win!',
      gameOver: 'Game Over',
      levelReached: 'Reached Level {level}/{max}',
      finalScores: 'Final Scores',
    },
    rules: {
      overviewTitle: 'What is The Mind?',
      overviewDesc:
        'The Mind is a cooperative card game where players must play numbered cards in ascending order to the center pile — without any communication. You win by clearing all levels together.',
      objectiveTitle: 'Objective',
      objectiveDesc:
        "Work together silently to play all cards from everyone's hand in ascending order across multiple levels. Clear all levels to win!",
      gameplayTitle: 'How to Play',
      gameplay1:
        'Each level, everyone receives cards equal to the level number (Level 1 = 1 card each, Level 5 = 5 cards each).',
      gameplay2:
        'Cards are sorted in your hand from lowest to highest. You may only play your lowest card.',
      gameplay3:
        'There are NO turns — anyone can play their lowest card at any time when they feel the timing is right.',
      gameplay4:
        'If someone plays a card and another player has a LOWER card in their hand, the team loses a life and all lower cards are discarded.',
      livesTitle: 'Lives',
      livesDesc:
        'The team starts with lives equal to the number of players. Each mistake costs 1 life. If lives reach 0, the game is over.',
      shurikenTitle: 'Shuriken (Throwing Stars)',
      shurikenDesc:
        'The team starts with 1 Shuriken. Any player can propose using it, and if all agree, everyone discards their lowest card. Use it wisely!',
      winTitle: 'Winning',
      winDesc:
        'Clear all levels to win! The number of levels depends on player count: 2 players = 12 levels, 3 = 10 levels, 4+ = 8 levels.',
      extremeTitle: 'Extreme Mode (2 Piles)',
      extremeDesc:
        'You must manage cards 1-100 on two piles: a White Pile (ascending) and a Red Pile (descending). You can jump backwards exactly 10 points (e.g. playing a 40 on a 50 White Pile)!',
      timeAttackTitle: 'Time Attack Mode',
      timeAttackDesc:
        'Adds a time limit to each level. If the timer runs out before you finish, you lose 1 life immediately!',
    },
  },
  gameSaboteur: {
    round: 'Round',
    stock: 'Deck',
    discard: 'Discard',
    emptyHand: 'No cards in hand',
    cancel: 'Cancel',
    rotate: 'Rotate card 180°',
    noPlacement: 'This card fits nowhere — try rotating, or discard it',
    waitingFor: "Waiting for {name}'s turn",
    brokenToolCanNotBuild: 'Broken tool! Repair it before building tunnels',
    blockedTitle: 'Broken tool — cannot build tunnels',
    blockedBanner:
      'Broken tool — you cannot build tunnels! Use a repair card, or discard / play another card',
    broken: 'broken',
    chooseBreakTarget: 'Choose a player whose tool to break',
    chooseRepairTarget: 'Choose a player whose tool to repair',
    chooseGoalToPeek: 'Click a goal card to peek at it',
    chooseRockfallCell: 'Click a tunnel tile to remove (start/goals are safe)',
    choosePlacement: 'Click a ghost tunnel to build it (⟳ to rotate)',
    selectCardHint: 'Pick a card from your hand',
    handSize: 'Cards in hand',
    players: 'Players',
    roleMiner: 'You are a Miner',
    roleMinerDesc: 'Dig together toward the gold',
    roleSaboteur: 'You are a Saboteur',
    roleSaboteurDesc: 'Sabotage secretly without getting caught',
    roleHidden: 'Role hidden',
    roleHiddenDesc: 'Press 👁️ to see your role',
    showRole: 'Show role',
    hideRole: 'Hide role',
    stoneEndsRound: 'Stone ends the round instantly',
    tool_lantern: 'lantern',
    tool_cart: 'cart',
    tool_pickaxe: 'pickaxe',
    logTitle: 'Play log',
    logPlace: '{name} built a tunnel at ({x},{y})',
    logBreak: "{name} broke {target}'s tool",
    logRepair: "{name} repaired {target}'s tool",
    logMap: '{name} peeked at goal shaft #{goal}',
    logRockfall: '{name} removed the tunnel at ({x},{y})',
    logDiscard: '{name} discarded a card',
    logPass: '{name} passed',
    logGold: '{name} picked 🪙{value} gold',
    logMinersWin: 'Miners struck gold at shaft #{goal}!',
    logSaboteursWin: 'The saboteurs win this round!',
    goldPickTitle: 'Gold found! Pick your nugget',
    yourPick: 'Your turn — click a gold card!',
    pickingNow: 'Now picking: {name}',
    minersWin: 'Miners win!',
    saboteursWin: 'Saboteurs win!',
    nextRound: 'Next round',
    waitingHostNextRound: 'Waiting for the host to start the next round...',
    gameOver: 'Game over! Winner(s)',
    backToLobby: 'Back to lobby',
    turnTimer: 'Turn timer',
    timerOn: 'On',
    timerOff: 'Off',
    secondsPerTurn: 'Seconds per turn',
    rules: {
      overviewTitle: 'Overview',
      overviewDesc:
        'Saboteur is a hidden-role mining game. Players are secretly Miners or Saboteurs. Miners cooperate to dig a tunnel from the start card to the gold; Saboteurs secretly sabotage them without being caught. Play 3 rounds — the player with the most gold wins.',
      rolesTitle: 'Roles',
      minerRole: '⛏️ Miner: wins as a team by connecting a path to the gold goal card',
      saboteurRole:
        '💣 Saboteur: wins as a team if the round ends without the gold being found (3 gold each, or 4 for a lone saboteur)',
      hiddenRoles:
        'The number of roles depends on the player count and is re-dealt every round, with one leftover role card kept hidden — nobody knows exactly how many enemies are out there.',
      turnTitle: 'On your turn, do exactly one of these',
      turnPath:
        'Play a path card: the tunnel must stay connected to the start, and openings must match on every touching side (rotate 180° as needed). You cannot build while any of your tools is broken.',
      turnAction: 'Play an action card on a player or the board.',
      turnDiscard: 'Discard one card and draw a replacement (while stock lasts).',
      actionTitle: 'Action cards',
      actionBreak:
        '🔨 Break: break one tool (🔦 lantern / 🛒 cart / ⛏️ pickaxe) of a target — a player with any broken tool cannot play path cards.',
      actionRepair:
        '🔧 Repair: fix a matching broken tool. Dual-tool repairs fix only one chosen tool.',
      actionMap:
        '🗺️ Map: privately peek at one goal card to see whether it hides gold or stone (only you see it).',
      actionRockfall:
        '🪨 Rockfall: remove one placed path tile from the board (start and goal cards are protected).',
      scoringTitle: 'Scoring',
      scoringDesc:
        'When a path reaches the gold card, it is revealed. The miners flip as many gold nugget cards as there are miners: the finder picks first, then picks continue counter-clockwise. After 3 rounds the richest player wins. If the deck runs dry first, the saboteurs win instantly.',
    },
  },
  gameCoup: {
    yourCoins: 'Your Coins',
    yourInfluence: 'Your Influence',
    coins: 'Coins',
    influences: 'Influence',
    turn: 'Turn',
    yourTurn: 'Your Turn!',
    waitingFor: 'Waiting for',
    deadPile: 'Dead Pile',
    deck: 'Deck',
    cards: 'cards',
    helpTitle: 'Ability Summary',
    helpButton: 'Abilities',
    roleDuke: 'Duke',
    roleDukeDesc: 'Tax +3 / Blocks Foreign Aid',
    roleAssassin: 'Assassin',
    roleAssassinDesc: 'Pay 3 to Assassinate 1',
    roleCaptain: 'Captain',
    roleCaptainDesc: 'Steal 2 / Blocks Steal',
    roleAmbassador: 'Ambassador',
    roleAmbassadorDesc: 'Exchange 2 / Blocks Steal',
    roleContessa: 'Contessa',
    roleContessaDesc: 'Blocks Assassination',
    actionsTitle: 'General Actions',
    actionIncome: 'Income +1',
    actionForeignAid: 'Foreign Aid +2 (blockable)',
    actionCoup: 'Coup Pay 7 to Kill',
    actionTax: 'Tax (Duke) +3',
    actionAssassinate: 'Assassinate (Assassin) Pay 3',
    actionSteal: 'Steal (Captain) 2 Coins',
    actionExchange: 'Exchange (Ambassador) Draw 2 Return 2',
    lobbyHint: 'Need at least 3 players to start Coup',
    need3Players: 'Need at least 3 players',
    spectating: 'Spectating',
    resetHint: 'Game over — reset to play again',
  },
  gameWhoKnow: {
    wordSettingHost: 'Please set the secret word using the popup.',
    wordSettingWaiting: 'Waiting for the Game Host to pick a word...',
    questioningPhase: 'Questioning Phase',
    questioningHost: "Answer the players' Yes/No questions.",
    questioningPlayers: 'Ask the Game Host Yes or No questions to find the Secret Word!',
    wordGuessedVote: 'Word Guessed (Vote)',
    timesUpFail: "Time's Up (Fail)",
    stopTimer: 'Stop Timer',
    votingPhase: 'Voting Phase',
    votingHostWait: 'Wait for the players to vote for the Insider.',
    votingPrompt: 'Who was secretly guiding the group? Cast your vote:',
    insider: 'Insider',
    resultsTitle: 'Game Results',
    timeoutTitle: "Time's Up!",
    timeoutDesc: 'The group failed to guess the secret word in time. Everyone loses.',
    insiderWinsTitle: 'Insider Wins!',
    insiderWinsDesc:
      'The group guessed the word, but failed to catch the Insider. The Insider scored 2 points!',
    commonersWinTitle: 'Commoners Win!',
    commonersWinDesc:
      'The group guessed the word and correctly caught the Insider. Commoners + Host scored +1 point!',
    secretWordWas: 'The Secret Word was',
    insiderWas: 'The Insider was',
    votingResults: 'Voting Results',
    mostVoted: 'Most Voted',
    votes: 'VOTES',
    vote: 'VOTE',
    playAgain: 'Play Again',
  },
};
