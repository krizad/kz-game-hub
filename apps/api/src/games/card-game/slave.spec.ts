...
const pad = (count: number): PlayingCard[] =>
  Array.from({ length: count }, (_, index) => card(`pad-${index}`, '5', 'CLUBS'));
...
describe('SlaveRuntime', () => {
  const runtimeFor = (popOrder: PlayingCard[]) =>
    new SlaveRuntime(new PrivateStateService(), () => [...popOrder].reverse());

  const config = SLAVE_PRESET.defaultConfig;
  const singleCardConfig = {
    ...config,
    deal: { ...config.deal, cardsPerPlayer: 1, countMode: 'EQUAL_WITH_LEFTOVERS' as const },
  };
  ...
  it('follows the preset starter policy when the deck holds no 3C', () => {
    ...
    expect(first.cardGameState!.activePlayerId).toBe('p1'); // ROTATE with no previous starter

    target.cardGameState = { ...first.cardGameState!, dealerId: 'p1' };
    const second = runtime.startRound(target, config, ['p1', 'p2']);
    expect(second.cardGameState!.activePlayerId).toBe('p2'); // ROTATE moves to the next seat
  });
  ...
  it('rejects an equal-rank follow and accepts a higher single', () => {
    ...
    const equalFollow = equalRuntime.handleAction(
      equalTarget,
      'p2',
      { type: 'PLAY', cards: ['3-SPADES', '3-HEARTS'] },
      config,
    );
    ...
    const higherFollow = higherRuntime.handleAction(
      higherTarget,
      'p2',
      { type: 'PLAY', cards: ['4-HEARTS'] },
      config,
    );
