...
import { CardRulePresetRepository } from './card-rule-preset.repository';
...
describe('CardRulePresetRepository', () => {
  let repository: CardRulePresetRepository;

  beforeEach(() => {
    jest.clearAllMocks();
    repository = new CardRulePresetRepository();
  });

  const validConfig = (): CardGameConfig =>
    JSON.parse(JSON.stringify(POK_DENG_PRESET.defaultConfig)) as CardGameConfig;
  ...
  describe('publish', () => {
    it('rejects a configuration outside the preset allow-list without touching storage', async () => {
      ...
      const result = await repository.publish(tampered, POK_DENG_PRESET);
    ...
    it('stores only the canonical validated config and returns an opaque code', async () => {
      ...
      const smuggled = { ...validConfig(), hands: ['A-CLUBS'] } as unknown as CardGameConfig;

      const result = await repository.publish(smuggled, POK_DENG_PRESET);
      ...
      expect((data.config as Record<string, unknown>).hands).toBeUndefined();
    ...
    it('retries with a fresh code when the share code collides', async () => {
      ...
      const result = await repository.publish(validConfig(), POK_DENG_PRESET);
  ...
  describe('importByCode', () => {
    it('returns NOT_FOUND for an unknown code and normalizes the lookup', async () => {
      ...
      const result = await repository.importByCode('  abcd23ef  ', POK_DENG_PRESET);
    ...
    it('refuses a code that belongs to another preset', async () => {
      ...
      const result = await repository.importByCode('ABC234XYZ89K', POK_DENG_PRESET);
    ...
    it('returns a room-local copy that cannot mutate the shared preset defaults', async () => {
      ...
      expect(POK_DENG_PRESET.defaultConfig.deal.cardsPerPlayer).toBe(2);
    });
  });
});
