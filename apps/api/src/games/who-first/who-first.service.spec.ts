import { WhoFirstService } from './who-first.service';
import { RoomState, RoomStatus } from '@repo/types';

describe('WhoFirstService', () => {
  let service: WhoFirstService;

  beforeEach(() => {
    service = new WhoFirstService();
  });

  function createRoom(config: Partial<RoomState['config']> = {}): RoomState {
    return {
      id: 'room-id',
      code: 'ABC123',
      gameType: 'WHO_FIRST',
